                                                  ...
                                                  const char *device_address)
{
    ...
    if (audio_is_output_device(device)) {
        ...
        case AudioSystem::DEVICE_STATE_AVAILABLE:
            ...
            ALOGD("mAvailableOutputDevices now 0x%x", mAvailableOutputDevices);
    ...
    if (audio_is_input_device(device)) {
        ...
        ALOGD("mAvailableInputDevices %x", mAvailableInputDevices);

        // limit input router to manage only a2dp input
        if (mAvailableInputDevices & AUDIO_DEVICE_IN_BLUETOOTH_A2DP)
        {
            ...
            if (mInputRouter != 0) {
                ALOGD("input router already open");
                return NO_ERROR;
            }

            // request output for MUSIC stream type
            output = getOutput(AudioSystem::MUSIC, hwOutputDesc->mSamplingRate,
                               AudioSystem::PCM_16_BIT, AudioSystem::CHANNEL_OUT_STEREO,
                               AudioSystem::OUTPUT_FLAG_INDIRECT);

            if (output == 0)
            {
                ALOGE("failed to get router output");
                return BAD_VALUE;
            }

            // request input using same rate as output
            input = getInput(AUDIO_SOURCE_A2DP, hwOutputDesc->mSamplingRate,
            ...
            if (input == 0)
            {
                ALOGE("failed to get router input");
                return BAD_VALUE;
            }

            ALOGV("opening input router (%d -> %d)", input, output);
            mInputRouter = mpClientInterface->openInputRouter(input, output);

            if (mInputRouter != 0) {
                ALOGD("successfully opened input router");

                /* need to register an output descriptor to allow APM to operate on this
                   output */

                AudioOutputDescriptor *outputDescr = new AudioOutputDescriptor(NULL);
                outputDescr->mOutput1 = mOutputs.valueFor(mPrimaryOutput);
                outputDescr->mOutput2 = mOutputs.valueFor(output);
                outputDescr->mSamplingRate = hwOutputDesc->mSamplingRate;
                outputDescr->mFormat = (audio_format_t)AudioSystem::PCM_16_BIT;
                outputDescr->mChannelMask = AudioSystem::CHANNEL_OUT_STEREO;
                outputDescr->mLatency = 0;
                addOutput(mInputRouter, outputDescr);
                applyStreamVolumes(mInputRouter, AUDIO_DEVICE_IN_BLUETOOTH_A2DP, 0, true);
            }
            else {
                ALOGE("input router failed to open");
            }
        }
        else
        {
            // close any opened input routers
            if (mInputRouter > 0)
            {
                ALOGD("release input router");
                releaseInput(mInputRouter);
                mInputRouter = 0;
            }
        }
...
void AudioPolicyManagerBase::setForceUse(AudioSystem::force_use usage, AudioSystem::forced_config config)
{
    ...
    case AudioSystem::FOR_COMMUNICATION:
        if (config != AudioSystem::FORCE_SPEAKER && config != AudioSystem::FORCE_BT_SCO &&
            config != AudioSystem::FORCE_BT_MIC &&
    ...
    case AudioSystem::FOR_RECORD:
        if (config != AudioSystem::FORCE_BT_SCO && config != AudioSystem::FORCE_WIRED_ACCESSORY &&
            config != AudioSystem::FORCE_BT_MIC && config != AudioSystem::FORCE_NONE &&
            config != AudioSystem::FORCE_BT_A2DP && config != AudioSystem::FORCE_NONE) {
    ...
    audio_io_handle_t activeInput = getActiveInput();
    ALOGV("activeInput = %d", activeInput);
    if (activeInput != 0) {
        AudioInputDescriptor *inputDesc = mInputs.valueFor(activeInput);
        audio_devices_t newDevice = getDeviceForInputSource(inputDesc->mInputSource);
        if ((newDevice != AUDIO_DEVICE_NONE) && (newDevice != inputDesc->mDevice)) {
...
AudioPolicyManagerBase::AudioPolicyManagerBase(AudioPolicyClientInterface *clientInterface)
    ...
    mLimitRingtoneVolume(false), mLastVoiceVolume(-1.0f),mLastFMVolume(-1.0f),mWithEarpiece(1),
    ...
    mInputRouter(0)
{
    ...
    if (property_get("audio.without.earpiece", prop_value, "0") > 0) {		
	if (atoi(prop_value) == 1) {
		mWithEarpiece = 0;
		ALOGVV("get property audio.without.earpiece: %d", mWithEarpiece);
	}
    }
    ...
    if (state == AudioSystem::DEVICE_STATE_AVAILABLE) {
        ...
        for (ssize_t profile_index = 0; profile_index < (ssize_t)profiles.size(); profile_index++) {
            ...
            if (output != 0) {
                if (desc->mFlags & AUDIO_OUTPUT_FLAG_DIRECT) {
                    ...
                    audio_io_handle_t inputRouter = 0;
                    audio_io_handle_t input = 0;
        ...
        for (size_t i = 0; i < mOutputs.size(); i++) {
            ...
            if (!desc->isDuplicated() &&
                    !(desc->mProfile->mSupportedDevices & mAvailableOutputDevices)&&(device!=AUDIO_DEVICE_OUT_BLUETOOTH_SPK)) {  //modified by oscar
...
{
    ALOGVV("=== checkOutputForStrategy strategy %d", strategy);
    ...
    ALOGVV("checkOutputForStrategy strategy %d oldDevice 0x%x, newDevice 0x%x", strategy, oldDevice, newDevice);

    if (!vectorsEqual(srcOutputs,dstOutputs)) {
        ALOGV("checkOutputForStrategy() strategy %d, moving from output %d to output %d",
              strategy, ((srcOutputs.isEmpty())?0:srcOutputs[0]),
              ((dstOutputs.isEmpty())?0:dstOutputs[0]));
        ...
        for (int i = 0; i < (int)AudioSystem::NUM_STREAM_TYPES; i++) {
            if (getStrategy((AudioSystem::stream_type)i) == strategy) {
                ...
                mpClientInterface->setStreamOutput((AudioSystem::stream_type)i,
                                 ((dstOutputs.isEmpty())?0:dstOutputs[0]) /* ignored */);
            }
        }
    }
}
    ...
    if (mA2dpSuspended) {
        if (((mScoDeviceAddress == "") ||
             ((mForceUse[AudioSystem::FOR_COMMUNICATION] != AudioSystem::FORCE_BT_SCO) &&
              ...
              (mForceUse[AudioSystem::FOR_COMMUNICATION] != AudioSystem::FORCE_BT_MIC) &&
              (mForceUse[AudioSystem::FOR_RECORD] != AudioSystem::FORCE_BT_MIC) &&
              (mForceUse[AudioSystem::FOR_MEDIA] != AudioSystem::FORCE_BT_MIC) &&
        ...
        if ((mScoDeviceAddress != "") &&
            ...
            (mForceUse[AudioSystem::FOR_RECORD] == AudioSystem::FORCE_BT_MIC) ||
            (mForceUse[AudioSystem::FOR_MEDIA] == AudioSystem::FORCE_BT_MIC))) {
                ALOGV("checkA2dpSuspend() Suspending a2dp");
                mpClientInterface->suspendOutput(a2dpOutput);
                mA2dpSuspended = true;
        }
    }
...
audio_devices_t AudioPolicyManagerBase::getNewDevice(audio_io_handle_t output, bool fromCache)
{
    ...
    ALOGVV("getNewDevice output %d, fromCache %d");
    AudioOutputDescriptor *outputDesc = mOutputs.valueFor(output);
                                                             ...
                                                             bool fromCache)
{
    ...
    if (fromCache) {
        ALOGD("getDeviceForStrategy() from cache strategy %d, device %x",
              strategy, mDeviceForStrategy[strategy]);
        return mDeviceForStrategy[strategy];
    }

    ALOGD("getDeviceForStrategy() strategy %d, device %x",
                  strategy, mDeviceForStrategy[strategy]);
    ...
    case STRATEGY_PHONE:
        ...
        case AudioSystem::FORCE_BT_MIC:
            device = mAvailableOutputDevices & AUDIO_DEVICE_OUT_BLUETOOTH_SPK;
            if (device) break;
    ...
    case STRATEGY_MEDIA: {
        ALOGVV("mHasA2dp %d, mForceUse 0x%x", mHasA2dp, mForceUse[AudioSystem::FOR_MEDIA]);
        uint32_t device2 = AUDIO_DEVICE_NONE;
	 ALOGE("mForceUse[AudioSystem::FOR_COMMUNICATION] = %d", mForceUse[AudioSystem::FOR_COMMUNICATION] );
	 ALOGE("mForceUse[AudioSystem::FOR_MEDIA] = %d", mForceUse[AudioSystem::FOR_MEDIA] );
	 ALOGE("mAvailableOutputDevices = %d", mAvailableOutputDevices);
	 if(mForceUse[AudioSystem::FOR_COMMUNICATION] == AudioSystem::FORCE_BT_MIC)
	 {
	 	device2 = mAvailableOutputDevices & AUDIO_DEVICE_OUT_BLUETOOTH_SPK;
	 
	 }
        else if (mForceUse[AudioSystem::FOR_MEDIA] == AudioSystem::FORCE_BT_MIC)
        {
            device2 = mAvailableOutputDevices & AUDIO_DEVICE_OUT_BLUETOOTH_SPK;
        } else if (strategy != STRATEGY_SONIFICATION) {
		...
		//ALOGD("lkj  2 media fm getDeviceForStrategy() mAvailableOutputDevices =%d" ,mAvailableOutputDevices );
	if ((mPhoneState == AudioSystem::MODE_FM) && (mForceUse[AudioSystem::FOR_COMMUNICATION] == AudioSystem::FORCE_SPEAKER)){//lkj
		device = mAvailableOutputDevices & AUDIO_DEVICE_OUT_SPEAKER;
		//ALOGD("lkj  3 media fm getDeviceForStrategy() mAvailableOutputDevices =%d" ,mAvailableOutputDevices );
		if (device) break;
	}
        if (device2 == AUDIO_DEVICE_NONE) {
                                             ...
                                             int delayMs)
{
    ...
    ALOGV("setOutputDevice() changing device, set routing %x on output %d", device, output);
    // do the routing
                                                   ...
                                                   uint32_t channelMask)
{
    ...
    ALOGV("getInputProfile rate %d, format %d, chmask %d", samplingRate, format, channelMask);
    for (size_t i = 0; i < mHwModules.size(); i++)
    {
        if (mHwModules[i]->mHandle == 0) {
            continue;
...
{
    uint32_t device = AUDIO_DEVICE_NONE;
    ALOGW("getDeviceForInputSource %d", inputSource);

    switch (inputSource) {
    case AUDIO_SOURCE_A2DP:
    {
        ALOGW("requested a2dp input device");
        device = AUDIO_DEVICE_IN_BLUETOOTH_A2DP;
        break;
    }/*
	case AUDIO_SOURCE_BLUETOOTH_MIC:
	{
		ALOGW("requested btmic input device");
    ...
    case AUDIO_SOURCE_MIC:
        if (mForceUse[AudioSystem::FOR_RECORD] == AudioSystem::FORCE_BT_A2DP &&
            mAvailableInputDevices & AUDIO_DEVICE_IN_BLUETOOTH_A2DP) {
            ALOGW("setup a2dp sink as audio source (RECORD)");
            device = AUDIO_DEVICE_IN_BLUETOOTH_A2DP;
            break;
        } else if (mForceUse[AudioSystem::FOR_MEDIA] == AudioSystem::FORCE_BT_A2DP &&
            mAvailableInputDevices & AUDIO_DEVICE_IN_BLUETOOTH_A2DP) {
            ALOGW("setup a2dp sink as audio source (MEDIA)");
            device = AUDIO_DEVICE_IN_BLUETOOTH_A2DP;
            break;
        }
    ...
    case AUDIO_SOURCE_VOICE_COMMUNICATION:
        ...
        if (mForceUse[AudioSystem::FOR_RECORD] == AudioSystem::FORCE_BT_MIC &&
            (inputSource == AUDIO_SOURCE_VOICE_RECOGNITION ||
            inputSource == AUDIO_SOURCE_MIC)) {
            /* Only for VoiceRecognition and MIC audio source, check for BT_MIC */
            device = AUDIO_DEVICE_IN_BLUETOOTH_MIC;
        }
        ALOGI("%s device:%x ", __FUNCTION__, device);
        break;
    case AUDIO_SOURCE_BT_MIC:
        if (mForceUse[AudioSystem::FOR_RECORD] == AudioSystem::FORCE_BT_MIC &&
            mAvailableInputDevices & AUDIO_DEVICE_IN_BLUETOOTH_MIC) {
            device = AUDIO_DEVICE_IN_BLUETOOTH_MIC;
        }
        break;
...
{
    ALOGD("getDeviceForVolume %x", device);
    if (device == AUDIO_DEVICE_NONE) {
...
*/
    ...
    if (stream == AudioSystem::FM && (mPhoneState == AudioSystem::MODE_FM)){
	    float fmVolume;
	    fmVolume = (float)index/(float)mStreams[stream].mIndexMax;
	    if (fmVolume != mLastFMVolume) {
		    mpClientInterface->setVoiceVolume(fmVolume, delayMs);
...
{
    ALOGV("isCompatibleProfile device %x mSupportedDevices %x", device, mSupportedDevices);
...
const struct StringToEnum sDeviceNameToEnumTable[] = {
    ...
    STRING_TO_ENUM(AUDIO_DEVICE_OUT_BLUETOOTH_SPK),
    ...
    STRING_TO_ENUM(AUDIO_DEVICE_IN_BLUETOOTH_MIC),
...
status_t AudioPolicyManagerBase::loadInput(cnode *root, HwModule *module)
{
    ...
    ALOGV("loadInput nodename %s = %s (HwModule %s)", node->name, node->value, module->mName);
...
void AudioPolicyManagerBase::loadHwModule(cnode *root)
{
    ...
    if (node != NULL) {
        ...
        } else if (strcmp(root->name, AUDIO_HARDWARE_MODULE_ID_BTMIC) == 0) {
            mHasBTMic = true;
        }
