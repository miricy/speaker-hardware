...
typedef enum
{
    BTHH_AUDIO_STATE_STOPPED             = 0,
    BTHH_AUDIO_STATE_STARTED
} bthh_audio_state_t;
...
typedef enum
{
    ...
    BTHH_ERR_HDL,
                                  /* OTA errors : */
    BTHH_ERR_OTA_NOT_SUPPORTED,
    BTHH_ERR_OTA_NO_MEM,
    BTHH_ERR_OTA_FILE_ERROR,
    BTHH_ERR_OTA_WRITE_ERROR,
    BTHH_ERR_OTA_READ_ERROR,
    BTHH_ERR_OTA_VERIFY_ERROR
}bthh_status_t;
...
/** Callback for set report api.
 */
typedef void (* bthh_set_report_callback)(bt_bdaddr_t *bd_addr, bthh_status_t hh_status);

/** Callback for HID Audio state change received via HID Report
 *  state will have one of the values from bthh_audio_state_t
 */
typedef void (* bthh_audio_state_callback)(bt_bdaddr_t *bd_addr, bthh_audio_state_t state);

/** Callback for OTA get firwmare version api.
 *  version contains the version value of the remote control firmware.
 */
typedef void (* bthh_ota_version_callback)(bt_bdaddr_t *bd_addr, bthh_status_t hh_status, uint16_t version);

/** Callback for OTA firwmare update api.
 * completion contains the percentage of the update process completion
 */
typedef void (* bthh_ota_update_callback)(bt_bdaddr_t *bd_addr, uint8_t completion);

/** Callback for OTA firwmare update api.
 * status indicates if the update was correctly done or not
 */
typedef void (* bthh_ota_updatedone_callback)(bt_bdaddr_t *bd_addr, bthh_status_t hh_status);

/** BT-HH callback structure. */
typedef struct {
    /** set to sizeof(BtHhCallbacks) */
    size_t      size;
    ...
    bthh_set_report_callback        set_report_cb;
