...
/** Bluetooth Adapter Device Modes*/
typedef enum {
    BT_DEFAULT_MODE,
    BT_HEADSET_MODE
} bt_device_mode_t;

/** Bluetooth Adapter SCO routing Modes*/
typedef enum {
    BT_SCO_ROUTE_PCM,
    BT_SCO_ROUTE_HCI
} bt_sco_route_mode_t;
...
/** Headless mode callback */
/* if status is BT_STATUS_SUCCESS, device has entered headless mode, failure otherwise */
typedef void (*headless_mode_changed_callback)(bt_status_t status);
...
typedef struct {
    ...
    headless_mode_changed_callback headless_mode_changed_cb;
} bt_callbacks_t;
